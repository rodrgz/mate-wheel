...
test('normalizes the wheel into a root tree and descriptor index', async () => {
  ...
  assert.equal(normalized.compositionCategories.length, source.compositionCategories.length);
  assert.equal(normalized.evaluationAxes.length, source.evaluationAxes.length);
  assert.deepEqual(normalized.descriptorIndex['mineralidade-retrogosto'].domains, ['flavor']);
  assert.deepEqual(normalized.descriptorIndex['menta'].intensityRange, [2, 4]);
  assert.deepEqual(normalized.descriptorIndex['grama-recem-cortada'].domains, ['flavor']);
});
