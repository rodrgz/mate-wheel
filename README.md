...
A versão beta assegura consistência garantindo que toda visualização da página vem de apenas uma fonte de verdade, o conteúdo processado no build time via Content Collections do Astro. Toda validação de esquemas de dados acontece de modo estrito antes do site ser publicado.
...
5. **Publicação**:
   A publicação no ar ocorre de modo automático sempre que código que passa pelo build é ingressado na `main`. O JSON exportador com os dados sensoriais canônicos normalizados ficará disponível em `/data/mate-wheel.json`.
...
Este projeto trata-se de explorações sensoriais pessoais e não se trata de nenhum estudo formal. Ele foi criado para ser uma roda de mate colaborativa. Sugestões de notas sensoriais são bem-vindas por [e-mail](mailto:mate@rodgz.com).
