export function normalizeMateWheel(source) {
  ...
  const compositionCategories = source.compositionCategories || [];
  const evaluationAxes = source.evaluationAxes || [];

  const descriptorIndex = {};
  const seenIds = new Set();

  const processDescriptor = (desc, familyId, inheritedMeta = {}) => {
    ...
    const nodeDomains = desc.domains || inheritedMeta.domains || [];
    ...
    if (desc.children && desc.children.length > 0) {
      node.children = desc.children.map((child) =>
        processDescriptor(child, familyId, { domains: nodeDomains })
      );
    } else {
      node.value = 1;
    }
