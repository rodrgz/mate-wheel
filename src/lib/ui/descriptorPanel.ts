  ...
  window.addEventListener(chartEvents.wheelNodeSelected, ((event: CustomEvent<WheelNodeSelectedDetail>) => {
    ...
    if (metaEl instanceof HTMLElement) {
      ...
      if (item.domains && item.domains.length > 0) {
        const localizedDomains = item.domains.map((domain: string) =>
          translations.domains?.[domain as keyof typeof translations.domains] || domain
        );
        fragment.appendChild(
          createMetaParagraph('meta-domains', localizedDomains.join(', '), translations.domainsLabel),
        );
      }
